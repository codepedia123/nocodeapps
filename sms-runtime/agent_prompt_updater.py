    ...
    try:
        ...
        def call_model(state: AgentState):
            ...
            sys_msg_content = (
                ...
                "- If the user explicitly asks to change when a tool should run, you must update the tool’s when_run scenarios, and if the user asks for an edit, fix, or behavioral change without mentioning tools or when_run, you must reason about the request at a behavioral level and decide whether adjusting a tool’s when_run scenarios would help satisfy or correctly implement the requested change, updating when_run only when the tool’s current execution conditions would otherwise prevent, misalign, or partially fulfill the user’s intent, and refraining from any when_run updates when the existing scenarios already support the requested behavior, since when_run decisions must be made dynamically based on intent and necessity, not applied automatically or by assumption. Prevent updating when_run everytime, but make it update only when it's necessary and helpful"
                "- If the user requests a change that affects execution order, conditions, confirmation steps, or dependencies between actions, you must first evaluate whether any tool’s when_run scenarios are now misaligned with the requested behavior, even if the user does not mention tools or when_run, and update when_run when tool execution would otherwise occur prematurely, automatically, or without required confirmation, using prompt edits only after when_run logic is correctly aligned, and never removing or altering prompt instructions as a substitute for fixing incorrect tool execution timing."
                "- If a behavioral request can be satisfied either by removing prompt instructions or by correcting tool execution logic, always prefer correcting tool execution logic first."
