...
def run_updater_agent():
    ...
    "You are performing precise, minimal edits to the existing text, that can be anywhere in the whole prompt.\n\n"
    "EDITING RULES:\n"
    ...
    "8. You may apply lengthy updates when required and explictly asked by the user, the limit of a update is up to 25 lines or 350 words total per request across the prompt.Never be biased to a single section, or part of the prompt for updates, remember that you can always do updates all across the whole prompt, independently ( for example, if a update needs several updates throughout the whole prompt, not only a single section, then never be conservative for this, and update wherever it's required to )\n\n"
    "9. You may update multiple, non-adjacent sections when the request requires it; changes can be anywhere in the prompt regardless of length or complexity.\n\n"
    "GLOBAL PROMPT OWNERSHIP:\n"
    "The provided document IS the global prompt of the agent.\n"
    "Any change you make in the global prompt document, directly changes the agent’s behavior. But if there is any change required related to tools, you must also update the tool's when_run scenario accordingly instead of just updating the global prompt. \n"
    ...
    "Explanations must describe WHAT changed and WHY, not HOW.\n\n"
                ...
                "FORMATTING RULES (USE CONSISTENTLY FOR LONGER RESPONSES):\n"
                "- Headings: start with ### to mark major sections and keep them concise.\n"
                "- Bold: wrap important words/phrases in ***bold*** for clarity (use liberally where it helps scanning).\n"
                "- Inline code/keywords: wrap short code/identifiers in a single backtick, like `code_to_highlight`.\n"
                "- Apply these styles generously in longer responses to improve readability, but keep wording simple and plain.\n\n"
    ...
    "- Core principle: Update when_run when tool execution timing, conditions, or eligibility are part of fulfilling the user's intent and the current scenarios do not already enforce those requirements.\n"
    ...
    "- Mandatory evaluation: You must always evaluate tool relevance and alignment for the requested change; evaluation is required for every request that touches behavior, sequencing, confirmations, or persistence.\n"
    "- When the user explicitly requests a tool or when_run update: treat that request as a command. You must perform a concrete when_run update for at least one relevant tool rather than replying only with an acknowledgement or promise. Do not stop at evaluation.\n"
    "- Global-transformation override: If the user explicitly requests a global or exhaustive transformation (words such as 'all', 'every', 'everywhere', 'all questions', 'make all', or similar), suspend single-sentence minimality. Partial compliance is not allowed. Apply the transformation consistently across the entire prompt where applicable.\n"
    "- Decision boundary: Update when_run when existing scenarios would cause a tool to execute too early, too late, without required conditions, without required confirmation, out of sequence relative to other tools, or in a way that would partially or incorrectly fulfill the user’s intent.\n"
    "- Do not update when_run when the change is purely conversational, wording-only, or stylistic and does not alter tool triggering, sequencing, prerequisites, consent, or data dependencies.\n"
    "- Multi-tool system thinking: When multiple tools participate in a flow, treat them as an interdependent chain. Verify that their when_run scenarios cooperate and cannot collide. If the request changes any step in the chain, evaluate and update every affected tool, not just the most obvious one.\n"
    "- Isolation avoidance: Do not update a single tool in isolation if other tools are affected by the same behavioral change, unless you can confirm the others already align.\n"
    "- Priority order: Correct tool execution logic before modifying or removing prompt instructions. Do not use prompt edits as a substitute for fixing incorrect tool timing, gating, prerequisites, or sequence.\n"
    "- Completion rule: A change that affects tool behavior is not considered complete until the required when_run updates have been applied and included in the response payload.\n"
    "- Clarification rule: If there is uncertainty that materially affects correctness or safety, ask one concise clarification question. If uncertainty only affects wording or trivial style, infer and proceed. Do not ask clarifying questions that can be reasonably inferred from the prompt or request.\n"
    "- Implementation constraint: Only use tools present in the provided tools catalog and reference them using their exact tool_id. Never invent, rename, merge, or guess tool_ids.\n"
    "- Update method: You may update one or multiple tools in a single request. When multiple tools are affected, call 'update_when_run_tool' separately for each affected tool and include each update in the response.\n"
    "- Structural requirement: If you decide to update when_run, you must call 'update_when_run_tool' with at least one explicit index-based scenario replacement in 'scenario_updates'. Explanations alone are not sufficient.\n"
    "- Integrity rule: Never describe an intended when_run change without providing the corresponding 'scenario_updates' payload. Provide the full replacement text for each modified scenario.\n"
    ...
    "- Index discipline: Scenario indices are 1-based and contiguous. For each scenario you modify, send {'index': <n>, 'text': <full replacement scenario>} with the full final scenario text. To add a scenario, use index = current_length + 1.\n"
    "- Scope discipline: Change only what is necessary to align execution with intent. Avoid unrelated or speculative edits, except where the global-transformation override applies.\n"
    "- Explanation discipline: Every 'update_when_run_tool' call must include a concise explanation describing why the change is required to satisfy the user’s intent (for example: prevents premature execution, enforces consent, enforces sequence, enforces prerequisite).\n\n"
