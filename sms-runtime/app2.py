...
# concurrency helper for running blocking agent logic off the event loop
import anyio
...
# Reuse a single session for connection pooling. Disable env proxy inheritance for internal calls.
...
# ---------------------------
def fetch_agent_details(agent_id: str, timeout: Optional[tuple] = None) -> Optional[Dict[str, Any]]:
    timeout = timeout or DEFAULT_TIMEOUT
    try:
        params = {"table": "agents", "id": agent_id}
        logger.log("fetch.agent.attempt", "Fetching agent details", {"agent_id": agent_id, "url": FETCH_BASE, "params": params})
        resp = HTTP_SESSION.get(FETCH_BASE, params=params, timeout=timeout, allow_redirects=True)
        resp.raise_for_status()
        try:
            data = resp.json()
        except ValueError as ve:
            logger.log("fetch.agent.parse_error", "JSON parse error", {"status": resp.status_code, "text_snippet": (resp.text[:800] if resp.text else ""), "error": str(ve)})
            return None
        logger.log("fetch.agent", "Fetched agent details", {"agent_id": agent_id, "status_code": resp.status_code})
        return data
    except requests.exceptions.Timeout as te:
        logger.log("fetch.agent.timeout", "Timeout while fetching agent details", {"agent_id": agent_id, "error": str(te)})
        return None
    except requests.exceptions.RequestException as re:
        logger.log("fetch.agent.request_error", "RequestException fetching agent details", {"agent_id": agent_id, "error": str(re)})
        return None
    except Exception as e:
        logger.log("fetch.agent.error", "Unexpected error fetching agent details", {"agent_id": agent_id, "error": str(e), "trace": traceback.format_exc()})
        return None

def fetch_agent_tools(agent_user_id: str, timeout: Optional[tuple] = None) -> Optional[Dict[str, Any]]:
    timeout = timeout or DEFAULT_TIMEOUT
    try:
        params = {"table": "all-agents-tools", "agent_id": agent_user_id}
        logger.log("fetch.tools.attempt", "Fetching agent tools", {"agent_user_id": agent_user_id, "url": FETCH_BASE, "params": params})
        resp = HTTP_SESSION.get(FETCH_BASE, params=params, timeout=timeout, allow_redirects=True)
        resp.raise_for_status()
        try:
            data = resp.json()
        except ValueError as ve:
            logger.log("fetch.tools.parse_error", "JSON parse error", {"status": resp.status_code, "text_snippet": (resp.text[:800] if resp.text else ""), "error": str(ve)})
            return None
        ...
        logger.log("fetch.tools.timeout", "Timeout while fetching agent tools", {"agent_user_id": agent_user_id, "error": str(te)})
        return None
    except requests.exceptions.RequestException as re:
        logger.log("fetch.tools.request_error", "RequestException fetching agent tools", {"agent_user_id": agent_user_id, "error": str(re)})
        return None
    except Exception as e:
        logger.log("fetch.tools.error", "Unexpected error fetching agent tools", {"agent_user_id": agent_user_id, "error": str(e), "trace": traceback.format_exc()})
        return None
        ...
        def make_api_call(tool_input: Any, url=api_url, tid=tool_id) -> str:
            ...
            if isinstance(tool_input, str):
                try:
                    ...
                    logger.log("tool.call.parse_error", "Invalid JSON provided to tool", {"tool_id": tid, "input_snippet": (tool_input[:400] if tool_input else "")})
                    return f"Error: Invalid JSON format."
            else:
                payload = tool_input

            logger.log("tool.call", f"api_tool_{tid} triggered", {"event_id": event_id, "payload": payload, "target_url": url})
            
            # Defensive defaults if url is empty
            if not url:
                logger.log("tool.call.error", "Missing api_url for tool", {"tool_id": tid})
                return f"API Call failed: tool configuration missing api_url."

            try:
                resp = HTTP_SESSION.post(url, json=payload, timeout=DEFAULT_TIMEOUT)
                resp.raise_for_status()
                # Try to present a short helpful success message
                text_snippet = resp.text[:800] if resp.text else ""
                logger.log("tool.call.success", "Tool API call succeeded", {"tool_id": tid, "status": resp.status_code, "resp_snippet": text_snippet})
                return f"Success: Data synced to API. status={resp.status_code}"
            except requests.exceptions.Timeout as te:
                logger.log("tool.call.timeout", "Timeout when calling tool API", {"tool_id": tid, "error": str(te), "url": url})
                return f"API Call failed: timeout"
            except requests.exceptions.RequestException as re:
                logger.log("tool.call.request_error", "RequestException when calling tool API", {"tool_id": tid, "error": str(re), "url": url})
                return f"API Call failed: {str(re)}"
            except Exception as e:
                logger.log("tool.call.error", "Unexpected error when calling tool API", {"tool_id": tid, "error": str(e), "trace": traceback.format_exc()})
...
async def run_endpoint(request: Request):
    ...
    # Run blocking run_agent off the event loop to avoid tying up the worker
    try:
        res = await anyio.to_thread.run_sync(lambda: run_agent(agent_id, conversation, message))
    except Exception as e:
        logger.log("endpoint.error", "Error while running agent in thread", {"error": str(e), "trace": traceback.format_exc()})
        return JSONResponse({"reply": f"Error: {str(e)}", "logs": logger.to_list()})
