...
def create_universal_tools(config: Dict[str, Any]) -> List[StructuredTool]:
    ...
    for tool_id, cfg in config.items():
        ...
        # Decode the template to show the LLM what the JSON should look like
        raw_payload_template = urllib.parse.unquote(cfg.get("api_payload_json", ""))

        # We use default arguments (url=api_url) to 'freeze' the values for each tool in the loop
        def make_api_call(url=api_url, tid=tool_id, **kwargs) -> str:
            """
            Dynamic tool handler. **kwargs captures all fields generated by the LLM.
            """
            event_id = str(uuid.uuid4())
            
            # 1. The payload is whatever the LLM generated (e.g., {"prompt": "..."})
            payload = kwargs
            
            # 2. Safety check: Remove internal LangChain keys if they accidentally leak into kwargs
            ...
            try:
                # 3. Send the payload exactly as the LLM constructed it
                ...
                err_result = {"ok": False, "tool_id": str(tid), "error": str(e)}
                logger.log("tool.error", f"api_tool_{tid} failed", err_result)
                return json.dumps(err_result, ensure_ascii=False)

        # 4. Refine the description so the LLM knows EXACTLY which keys to create
        tool_desc = (
            f"WHEN TO USE: {condition}. "
            f"REQUIRED FIELDS: {raw_payload_template}. "
            f"INSTRUCTIONS: {instructions}. "
            "You must provide a JSON object matching the REQUIRED FIELDS."
        )
